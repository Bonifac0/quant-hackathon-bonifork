...
class AiRegresor(IAi):  # stara Michalovo B. verze
    ...
    def get_probabilities(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        return self._calculate_probabilities(predicted_score_differences)

    def save_model(self, path: os.PathLike) -> None:
        """Save ML model."""
        self.model.save_model(path)

    def _home_team_win_probability(self, score_difference: float) -> float:
    ...
    def _calculate_probabilities(self, score_differences: np.ndarray) -> pd.DataFrame:
        """Calculate the probabilities of teams winning based on score differences."""
        probabilities = []

        for score_difference in score_differences:
            home_prob = self._home_team_win_probability(score_difference)
